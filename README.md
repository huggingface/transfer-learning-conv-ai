...
## Migration update
Updated for migration from `pytorch_pretrained_bert` to `pytorch_transformers`. `python interact.py` on its own is not currently compatible with the pretrained weights. Check `migration_notes.md`. 
