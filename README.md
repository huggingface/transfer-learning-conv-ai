...
This code is a simple, clean and commented code base with training and testing scripts that can be used to build a State-of-the-Art dialog agent based on Transfer Learning from an OpenAI GPT and GPT-2 Transformer language model.

This codebase can be used to reproduce the results of HuggingFace's participation to NeurIPS 2018 dialog competition [ConvAI2](http://convai.io/) which won the automatic metrics track. The 3k+ lines of competition code was distilled in less than 250 lines of clean and commented training code (with distributed & FP16 options) to form the present repository.

This model can be trained for less than $10 on a cloud instance and a pre-trained model is also made available.
